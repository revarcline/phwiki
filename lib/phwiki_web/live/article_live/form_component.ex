defmodule PhwikiWeb.ArticleLive.FormComponent do
  ...
  def handle_event("validate", %{"article" => article_params}, socket) do
    changeset =
      ...
      |> Wiki.change_article(article_params)
  ...
  defp save_article(socket, :edit, article_params) do
    case Wiki.update_article(socket.assigns.article, article_params) do
      {:ok, _article} ->
        {:noreply,
         socket
  ...
  defp save_article(socket, :new, article_params) do
    case Wiki.create_article(article_params) do
      {:ok, _article} ->
        {:noreply,
         socket
